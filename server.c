...
#  include <CommonCrypto/CommonDigest.h>
   static void plat_sha1(const uint8_t *msg, size_t len, uint8_t out[20]) {
...
static char *plat_b64(const uint8_t *d, size_t len) {
    static const char T[] =
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    size_t outlen = ((len + 2) / 3) * 4 + 1;
    char *buf = malloc(outlen);
    size_t i, j;
    for (i = 0, j = 0; i < len; ) {
...
/* WebSocket handshake – używa plat_sha1 + plat_b64 */
static int ws_handshake(int sock, char *buf) {
    ...
    uint8_t digest[20];
    plat_sha1((const uint8_t *)combined, strlen(combined), digest);
    char *accept = plat_b64(digest, 20);
    ...
    free(accept);
    return (write(sock, resp, rlen) > 0) ? 0 : -1;
}
