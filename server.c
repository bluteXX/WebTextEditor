 ...
 *
 * Serwer obsługuje:
 *   - GET /        → zwraca stronę HTML z edytorem
 *   - GET / + Upgrade: websocket → WebSocket handshake
 */
 ...
 /* ── HTML strony ── */
 /* JS automatycznie wykrywa ws:// lub wss:// na podstawie protokołu strony */
 ...
 "</main>\n"
 "<script>\n"
 "/* Automatycznie wykryj ws:// lub wss:// na podstawie protokołu strony */\n"
 ...
 static void *connection_handler(void *arg){
     ...
     /* Odbierz żądanie HTTP */
     char buf[4096]={0};
     int n=recv(sock,buf,sizeof(buf)-1,0);
     if(n<=0){close(sock);pthread_exit(NULL);}
 
     /* Rozróżnij HTTP GET od WebSocket */
     if(ws_handshake(sock, buf)!=0){
         /* Był to zwykły HTTP GET — HTML już wysłany, zamknij */
         close(sock);
         pthread_exit(NULL);
     }
 
     /* Od tej pory to połączenie WebSocket */
     client_add(sock);
     fprintf(stderr,"[+] Klient fd=%d połączony. Aktywni: %d\n",sock,client_count);
 
     /* Wyślij aktualny stan dokumentu */
